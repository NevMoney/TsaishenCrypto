...
function houseBox(id, isMarketplace, price, owner) {
    var houseDiv = `<div class="col-lg-3 houseBox m-5 light-b-shadow" id="portfolioDisplay${id}">
                        <div class="house" onclick="selectHouse(${id})">
                            ...
                            <button class="btn btn-warning light-b-shadow" id="buyWithETH${id}" onclick="selectHouseToBuyWEth(${id})">Buy ETH ${price}</button>
                            <button class="btn btn-warning light-b-shadow" id="buyWithETHEscrow${id}" onclick="selectHouseToBuyWEth(${id})">Escrow Buy ETH ${price}</button>
                            <button class="btn btn-warning light-b-shadow" id="buyWithUSDC${id}" onclick="selectHouseToBuyWUsdc(${id})">Buy USDC ${price}</button>
                            <button class="btn btn-warning light-b-shadow" id="buyWithUSDCEscrow${id}" onclick="selectHouseToBuyWUsdc(${id})">Escrow Buy USDC ${price}</button>
